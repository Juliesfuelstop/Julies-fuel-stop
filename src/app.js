...
const MenuItem = ({ item, onAddToCart, onRemoveFromCart, isInCart }) => (
  React.createElement("div", { className: "bg-white p-4 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 transform hover:scale-105" },
    React.createElement("h3", { className: "text-lg font-semibold text-gray-800" }, item.name),
    React.createElement("p", { className: "text-gray-600 mb-2" }, "$", item.price.toFixed(2)),
...
const Cart = ({ cartItems, removeFromCart, taxRate, tipAmount, setTipAmount, onPay }) => {
  ...
  return (
    React.createElement("div", { className: "p-6 bg-white rounded-lg shadow-md max-w-md mx-auto" },
      React.createElement("h2", { className: "text-3xl font-bold mb-6 text-center text-indigo-600" }, "Cart"),
      React.createElement("div", { className: "mb-4" },
        cartItems.length ? (
          cartItems.map((item) =>
            React.createElement("p", { className: "flex justify-between text-gray-800", key: item.id },
              React.createElement("span", null,
                item.name, ": $", item.price.toFixed(2)
              ),
              React.createElement("button", { className: "text-red-500 hover:underline", onClick: () => removeFromCart(item.id) }, "Remove")
            )
          )
        ) : (
          React.createElement("p", { className: "text-gray-600" }, "No items in cart.")
        )
      ),
      React.createElement("div", { className: "border-t pt-4" },
        React.createElement("p", { className: "flex justify-between text-gray-800" },
        ...
        React.createElement("div", { className: "flex justify-between items-center mb-4" },
          React.createElement("span", { className: "text-gray-800" }, "Tip:"),
          React.createElement("div", null,
            React.createElement("button", { className: "bg-indigo-500 text-white px-3 py-1 rounded mr-2 hover:bg-indigo-600 transition", onClick: () => setTipAmount(subtotal * 0.05) }, "5%"),
            React.createElement("button", { className: "bg-indigo-500 text-white px-3 py-1 rounded mr-2 hover:bg-indigo-600 transition", onClick: () => setTipAmount(subtotal * 0.10) }, "10%"),
            React.createElement("button", { className: "bg-indigo-500 text-white px-3 py-1 rounded mr-2 hover:bg-indigo-600 transition", onClick: () => setTipAmount(subtotal * 0.15) }, "15%"),
            React.createElement("input", {
              type: "number",
              placeholder: "Custom $",
        ...
        React.createElement("p", { className: "flex justify-between font-bold text-gray-800" },
          React.createElement("span", null, "Total:"),
          React.createElement("span", null, "$", total.toFixed(2))
        )
      ),
      React.createElement("div", { className: "mt-6" },
        React.createElement("h3", { className: "text-xl font-semibold mb-4 text-indigo-600" }, "Payment"),
        ...
        React.createElement("button", { className: "bg-indigo-600 text-white px-4 py-2 rounded w-full hover:bg-indigo-700 transition", onClick: () => onPay(total) }, "Pay Now")
      )
    )
  );
};
...
const Navbar = ({ cartItems }) => {
  return (
    React.createElement("nav", { className: "bg-indigo-600 text-white p-4 flex justify-between items-center shadow-md" },
      React.createElement("div", { className: "text-2xl font-bold" }, "Tasty Bites"),
      React.createElement("div", null,
        React.createElement(Link, { to: "/", className: "mr-4 hover:underline" }, "Home"),
        React.createElement(Link, { to: "/menu", className: "mr-4 hover:underline" }, "Menu"),
        React.createElement(Link, { to: "/cart", className: "relative hover:underline" },
          "Cart",
          cartItems.length > 0 && (
            React.createElement("span", { className: "absolute -top-1 -right-1 bg-red-500 text-white rounded-full h-5 w-5 flex items-center justify-center text-xs" },
              cartItems.length
            )
          )
        )
...
const MenuPage = ({ onAddToCart, onRemoveFromCart, cartItems }) => {
  ...
  const filteredItems = menuItems.filter(item => item.category.startsWith(activeTab));

  return (
    React.createElement("div", { className: "p-6" },
      React.createElement("h2", { className: "text-3xl font-bold mb-6 text-center text-indigo-600" }, "Our Menu"),
      React.createElement("div", { className: "flex justify-center mb-6" },
        categories.map(category => (
          React.createElement("button", {
            key: category,
            className: `px-4 py-2 mx-2 rounded-lg font-semibold transition-all duration-300 ${activeTab === category ? "bg-indigo-600 text-white" : "bg-gray-200 text-gray-800 hover:bg-gray-300"}`,
...
const HomePage = () => (
  React.createElement("div", { className: "p-6 text-center" },
    React.createElement("h2", { className: "text-4xl font-bold mb-4 text-indigo-600" }, "Welcome to Tasty Bites!"),
    React.createElement("p", { className: "text-gray-700 mb-6" }, "Explore our delicious menu and enjoy your meal!"),
    React.createElement("div", { className: "mb-6" },
      React.createElement("h3", { className: "text-2xl font-semibold mb-2 text-indigo-600" }, "Promotions"),
      promotions.map((promo, index) =>
        React.createElement("p", { key: index, className: "text-gray-700" }, promo)
      )
    ),
    React.createElement(Link, { to: "/menu", className: "bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition" }, "View Menu")
  )
);
...
const App = () => {
  ...
  return (
    React.createElement(BrowserRouter, null,
      React.createElement("div", { className: "min-h-screen bg-gradient-to-br from-indigo-100 to-gray-100 font-sans" },
        React.createElement(Navbar, { cartItems }),
        React.createElement("div", { className: "container mx-auto" },
          React.createElement(Switch, null,
            React.createElement(Route, { exact: true, path: "/" },
              React.createElement(HomePage, null)
            ),
            React.createElement(Route, { path: "/menu" },
